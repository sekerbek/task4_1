...
//The number of the thread that executes this code is stored in rank, and the number of processes is stored in size.                    
    int rank, size;
...
// here in order to partion the image into blocks we divide the mysizeY/size
// so the block gonna to go throug each row 
// its the size of the block like the part of the image that will be processed by one thread
//
    int mySizeX = viewPlaneResolutionX;
    int mySizeY = viewPlaneResolutionY/size;

    // Each process stores its piece only.
//here will be created the piece of image that gonna computed by one thread
    Image myPiece(mySizeX, mySizeY);
    
// start to calculate the time that will compute in this image
    double ts = MPI_Wtime();
...
//here the image itself is calculated. The calculation is pixel by pixel-  x and y are pixel coordinates
// the image gonna give the color for each piece of image
// in order to do not overlap each piece of image we use the y+ mysizeY*rank
//
	...
	//we assign to our piece of the picture the color that was calculated
        myPiece.set(x, y, color);
    }

// calculate the time it took to calculate a piece of the image
    double te = MPI_Wtime();
...
//output to the console the maximum execution time for the thread (which will be the final running time of the algorithm)
    double max_time;
    ...
    //number 0 - main thread where different pieces merge into one picture
    if (rank == 0) {

        // This will be the final image. (but now it is empty, there is no color in it)
